% This script assembles a time series for each JIM-microstructure-microwave model combination
% Want to get a list of directories in the results folder
% Define some top-level folder.
topLevelFolder = '../TBresults_MainFarm/'
basepath = pwd
...
for d = 1:size(listOfFolderNames,2)
    ...
    if ~isempty(dir('*.out'))
        ...
        for i=1:63
            ...
            % MOSES
            mos = strcat('*_',x,'_MOS.dat*.out');
            mosoutput = strcat('Processed/MOS_',x,'.txt');
            FileList = dir(mos);
            N = size(FileList,1);
            ...
            save(mosoutput, 'TBresults', '-ascii');

            % SNICAR
            sni = strcat('*_',x,'_SNI.dat*.out');
            snioutput = strcat('Processed/SNI_',x,'.txt');
            FileList = dir(sni);
            N = size(FileList,1);
            % Initialise TBresults
            TBresults = [];
            ...
            % Write results to file
            save(snioutput, 'TBresults', '-ascii');

            % SNTHERM
            snt = strcat('*_',x,'_SNT.dat*.out');
            sntoutput = strcat('Processed/SNT_',x,'.txt');
            FileList = dir(snt);
            N = size(FileList,1);
            % Initialise TBresults
            TBresults = [];
            ...
            % Write results to file
            save(sntoutput, 'TBresults', '-ascii');
